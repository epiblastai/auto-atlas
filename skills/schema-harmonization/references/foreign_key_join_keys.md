...
4. **Polymorphic FKs split by variant.** A `PolymorphicForeignKeyField` targets several schemas; identify and record a separate join key per variant target, each against its own registry.
...
If no valid join key exists and the foreign key cannot be left null, surface that to the user rather than recording a key that does not validate.
...
- **Referencing table:** `{field_name}_{target_schema}_join` — holds the natural-key value(s) that identify the target row(s), in the foreign key's cardinality (scalar for a scalar FK, list for a list FK). For a `PolymorphicForeignKeyField`, write one column per variant, each suffixed with that variant's target schema.
...
Leave the value columns these keys reference untouched otherwise — finalization reads the `*_join` columns, resolves them to `uid`s, fills the foreign key fields, and drops the `*_join` columns as part of finalizing the table.
