...
- Prefer `OntologyAlignedField` for ontology terms and `CrossReferenceField` for external database IDs. Pass values from `auto_atlas.registries.OntologyRegistry` and `CrossReferenceDbRegistry` — not free-form strings — so resolution tooling can look up the correct resolver.
